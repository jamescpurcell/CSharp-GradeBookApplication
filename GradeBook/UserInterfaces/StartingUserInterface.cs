...
namespace GradeBook.UserInterfaces
{
    public static class StartingUserInterface
    {
        ...
        public static void CreateCommand(string command)
        {
            ...
            var stringGradeBook = parts[2];
            bool IsWeighted = Boolean.Parse(parts[3]);
            BaseGradeBook gradeBook;
            if (stringGradeBook == "Standard")
            {
                gradeBook = new StandardGradeBook(name, IsWeighted);
            }
            else if (stringGradeBook == "ranked")
            {
                gradeBook = new RankedGradeBook(name, IsWeighted);
            }
            else
            {
                Console.WriteLine($"{stringGradeBook} is not a supported type of gradebook, please try again");
                return;
            }
